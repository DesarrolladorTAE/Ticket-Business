...
const nombreCompleto = (client) =>
  `${client?.name || ""} ${client?.apellido_paterno || ""} ${
  ...
  return (
    ...
    >
      <CardContent>
        <Typography
          variant="body2"
          sx={{ color: "#64748b", fontWeight: 700, mb: 1 }}
        >
          {title}
        </Typography>
        ...
        {description && (
          <Typography
            variant="caption"
            sx={{ display: "block", color: "#64748b", mt: 1 }}
          >
            {description}
          </Typography>
        )}
...
function ClienteFormFields({ formulario, setFormulario, cargando }) {
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          ...
          {client?.origin === "integration" && (
            <Alert severity="info">
              Este cliente fue registrado mediante una integración externa.
            </Alert>
          )}
...
function ConfirmarEstadoDialog({
  ...
  const activo = Number(client?.company_status) === 1;
  ...
  return (
    ...
    >
      <DialogTitle sx={{ fontWeight: 900 }}>
        {activo ? "Inactivar cliente" : "Activar cliente"}
      </DialogTitle>
        ...
        <Stack spacing={2}>
          ...
          <Paper variant="outlined" sx={{ p: 1.5, borderRadius: 2, bgcolor: "#f8fafc" }}>
            ...
            {client?.email && (
              <Typography
                variant="body2"
                sx={{ color: "#64748b", mt: 0.3, wordBreak: "break-word" }}
              >
                {client.email}
              </Typography>
            )}
          ...
          <Alert severity="warning">
            Si el cliente ya tiene historial de tickets, el sistema conservará
            la información necesaria para no perder ese historial.
          </Alert>
        </Stack>
      </DialogContent>
...
function ClienteCard({
  ...
  const activo = Number(client.company_status) === 1;
  const cambiando = Number(changingStatusId) === Number(client.id);
  const eliminando = Number(deletingClientId) === Number(client.id);
      ...
      <Stack spacing={2}>
        <Stack direction="row" spacing={1.5}>
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            <Typography sx={{ fontWeight: 900, color: "#0f172a" }}>
              {nombreCompleto(client) || "Sin nombre"}
            </Typography>
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap">
          <Chip
            label={client.origin === "integration" ? "Integración" : "Registrado"}
            variant="outlined"
            size="small"
            color={client.origin === "integration" ? "secondary" : "primary"}
          />
        ...
        >
          {[
            ...
            ["Abiertos", client.open_tickets],
            ["Resueltos", client.resolved_tickets],
          ].map(([label, value]) => (
            <Paper key={label} variant="outlined" sx={{ p: 1 }}>
              <Typography variant="caption" color="text.secondary">
                {label}
              </Typography>
              <Typography fontWeight={900}>{formatNumber(value)}</Typography>
            </Paper>
          ))}
        </Box>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
          ...
          <Button
            ...
            sx={{ textTransform: "none", fontWeight: 800, boxShadow: "none" }}
          >
            {cambiando ? "Procesando..." : activo ? "Inactivar" : "Activar"}
          </Button>
...
export default function Clientes() {
  ...
  const loadClients = async ({ refresh = false } = {}) => {
    ...
    try {
      const response = await axiosCliente.get("/clients/summary");
  ...
  const clientesFiltrados = useMemo(() => {
    ...
    return clients.filter((client) => {
      const nombre = normalizarTexto(nombreCompleto(client));
      const email = normalizarTexto(client.email);
      const telefono = normalizarTexto(client.telefono);
      ...
      const coincideOrigen =
        originFilter === "all" || client.origin === originFilter;

      return coincideBusqueda && coincideEstado && coincideOrigen;
    });
  }, [clients, search, statusFilter, originFilter]);
  ...
  const confirmarEstado = async () => {
    if (!statusClient?.id) return;

    const activo = Number(statusClient.company_status) === 1;
    ...
    try {
      await axiosCliente.patch(`/clients/${statusClient.id}/status`, {
        status: nuevoStatus,
      });
  ...
  const confirmarEliminar = async () => {
    if (!deleteClient?.id || deletingClient) return;
    ...
    try {
      const response = await axiosCliente.delete(
        `/clients/${deleteClient.id}`,
      );

      setSuccess(
        response.data?.message || "Cliente eliminado correctamente.",
      );
  ...
  const handleCreated = async (message) => {
    setSuccess(message || "Cliente registrado correctamente.");
    setError("");
    await loadClients({ refresh: true });
  };
  ...
  return (
    <Box sx={{ width: "100%", overflowX: "hidden" }}>
      <Stack spacing={3}>
        ...
        >
          <Box>
            ...
            >
              Consulta y administra los clientes registrados en la empresa.
            </Typography>
          </Box>
        ...
        >
          ...
          <MetricCard
            ...
            value={summary.inactive_clients}
            description="Clientes con acceso inactivo"
          />
          <MetricCard
            title="Con tickets"
            value={summary.clients_with_tickets}
            description="Clientes que ya tienen historial"
          />
          <MetricCard
            title="Integraciones"
            value={summary.integration_clients}
        ...
        >
          <Box>
            <Typography fontWeight={900}>Lista de clientes</Typography>
            <Typography variant="body2" color="text.secondary">
              Consulta información y actividad de cada cliente.
            </Typography>
          </Box>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
            <Button
              ...
              disabled={refreshing}
              onClick={() => loadClients({ refresh: true })}
              sx={{ textTransform: "none", fontWeight: 800 }}
            >
        ...
        {clientesFiltrados.length === 0 ? (
          ...
          >
            <Typography fontWeight={900}>No se encontraron clientes.</Typography>
          </Paper>
        ) : isMobile ? (
          <Stack spacing={2}>
            {clientesPaginados.map((client) => (
              <ClienteCard
                ...
                client={client}
                onEdit={abrirEditar}
                onToggleStatus={(item) => setStatusClient(item)}
                changingStatusId={changingStatus ? statusClient?.id : null}
                onDelete={abrirEliminar}
                deletingClientId={deletingClient ? deleteClient?.id : null}
              />
            ))}
              ...
              <Table stickyHeader>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 900 }}>Contacto</TableCell>
                    <TableCell sx={{ fontWeight: 900 }}>Origen</TableCell>
                    <TableCell sx={{ fontWeight: 900 }}>Estado</TableCell>
                    <TableCell sx={{ fontWeight: 900 }}>Tickets</TableCell>
                    <TableCell sx={{ fontWeight: 900 }}>Acciones</TableCell>
                  </TableRow>
                </TableHead>
                        ...
                        <TableCell>
                          <Stack spacing={0.25}>
                            <Typography variant="caption">
                              Total:{" "}
                              <strong>{formatNumber(client.total_tickets)}</strong>
                            </Typography>
                            <Typography variant="caption">
                              Abiertos:{" "}
                              <strong>{formatNumber(client.open_tickets)}</strong>
                            </Typography>
                            <Typography variant="caption">
                              Resueltos:{" "}
                              <strong>
                                {formatNumber(client.resolved_tickets)}
                              </strong>
                            </Typography>
                          </Stack>
                        </TableCell>
                        ...
                        <TableCell>
                          <Stack spacing={0.75}>
                            ...
                            <Button
                              ...
                              disabled={
                                changingStatus &&
                                Number(statusClient?.id) === Number(client.id)
                              }
                              sx={{
                                textTransform: "none",
                                fontWeight: 800,
                            ...
                            <Button
                              ...
                              color="error"
                              startIcon={<DeleteIcon />}
                              onClick={() => abrirEliminar(client)}
                              disabled={
                                deletingClient &&
                                Number(deleteClient?.id) === Number(client.id)
                              }
                              sx={{ textTransform: "none", fontWeight: 800 }}
                            >
                              {deletingClient &&
      ...
      <ConfirmarEstadoDialog
        ...
        onClose={() => {
          if (!changingStatus) setStatusClient(null);
        }}
